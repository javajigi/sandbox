...
public class HTTPPOSTServer extends Thread {
				...
				do {
					...
					if (currentLine
						...
						if (Long.valueOf(contentLength) > 2000000L) {
							sendResponse(200, "File size should be < 2MB",
									false);
						}
						...
						String fileContentType = inFromClient.readLine().split(
								" ")[1];
						...
						while (true) {
							if (currentLine.equals("--" + boundary + "--")) {
								System.out.println("boundary : " + prevLine);
								fout.print(prevLine);
								break;
							} else {
								System.out.println("File Contents : " + prevLine);
	...
	public void sendResponse(int statusCode, String responseString,
		...
		if (isFile) {
			...
			responseString = HTTPPOSTServer.HTML_START + responseString
					+ HTTPPOSTServer.HTML_END;
